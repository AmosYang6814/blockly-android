...
Blockly.PHP.scrub_ = function (a, b, c) {
    ...
    if (!a.outputConnection || !a.outputConnection.targetConnection) {
        ...
        (e = Blockly.utils.wrap(e, Blockly.PHP.COMMENT_WRAP - 3)) && (d += Blockly.PHP.prefixLines(e, "// ") + "\n");
...
Blockly.PHP.lists_indexOf = function (a) {
    ...
    if (a.workspace.options.oneBasedIndex) var d = " 0", e = " + 1"; else d = " -1", e = "";
...
Blockly.PHP.lists_setIndex = function (a) {
    ...
    switch (c) {
        case "FIRST":
            ...
            break;
        case "LAST":
            c = Blockly.PHP.valueToCode(a, "LIST", Blockly.PHP.ORDER_COMMA) ||
                "array()";
...
Blockly.PHP.lists_getSublist = function (a) {
    var b = Blockly.PHP.valueToCode(a, "LIST", Blockly.PHP.ORDER_COMMA) || "array()", c = a.getFieldValue("WHERE1"),
    ...
    if ("FIRST" != c || "LAST" != d) if (b.match(/^\$\w+$/) || "FROM_END" != c && "FROM_START" == d) {
        ...
        "  }", "  $length = 0;", "  if ($where2 == 'FROM_START') {", "    $length = $at2 - $at1 + 1;", "  } else if ($where2 == 'FROM_END') {", "    $length = count($list) - $at1 - $at2;", "  } else if ($where2 == 'LAST') {", "    $length = count($list) - $at1;", "  } else {", "    throw new Exception('Unhandled option (lists_get_sublist).');", "  }", "  return array_slice($list, $at1, $length);", "}"]) + "(" + b + ", '" + c + "', " + e + ", '" + d + "', " + a + ")";
    return [b, Blockly.PHP.ORDER_FUNCTION_CALL]
};
Blockly.PHP.lists_sort = function (a) {
    ...
    return [Blockly.PHP.provideFunction_("lists_sort", ["function " + Blockly.PHP.FUNCTION_NAME_PLACEHOLDER_ + "($list, $type, $direction) {", "  $sortCmpFuncs = array(", '    "NUMERIC" => "strnatcasecmp",', '    "TEXT" => "strcmp",', '    "IGNORE_CASE" => "strcasecmp"', "  );", "  $sortCmp = $sortCmpFuncs[$type];", "  $list2 = $list;", "  usort($list2, $sortCmp);",
        "  if ($direction == -1) {", "    $list2 = array_reverse($list2);", "  }", "  return $list2;", "}"]) + "(" + b + ', "' + a + '", ' + c + ")", Blockly.PHP.ORDER_FUNCTION_CALL]
};
...
Blockly.PHP.controls_repeat_ext = function (a) {
    ...
    a = "";
    var d = Blockly.PHP.variableDB_.getDistinctName("count", Blockly.Variables.NAME_TYPE), e = b;
...
Blockly.PHP.controls_for = function (a) {
    ...
    f = Blockly.PHP.addLoopTrap(f, a.id);
...
Blockly.PHP.math_number_property = function (a) {
    ...
    if ("PRIME" == c) return [Blockly.PHP.provideFunction_("math_isPrime", ["function " + Blockly.PHP.FUNCTION_NAME_PLACEHOLDER_ + "($n) {", "  // https://en.wikipedia.org/wiki/Primality_test#Naive_methods", "  if ($n == 2 || $n == 3) {", "    return true;", "  }", "  // False if n is NaN, negative, is 1, or not whole.", "  // And false if n is divisible by 2 or 3.",
        "  if (!is_numeric($n) || $n <= 1 || $n % 1 != 0 || $n % 2 == 0 || $n % 3 == 0) {", "    return false;", "  }", "  // Check all the numbers of form 6k +/- 1, up to sqrt(n).", "  for ($x = 6; $x <= sqrt($n) + 1; $x += 6) {", "    if ($n % ($x - 1) == 0 || $n % ($x + 1) == 0) {", "      return false;", "    }", "  }", "  return true;", "}"]) + "(" + b + ")", Blockly.JavaScript.ORDER_FUNCTION_CALL];
    switch (c) {
        case "EVEN":
            var d = b + " % 2 == 0";
            break;
    ...
    switch (b) {
        ...
        case "STD_DEV":
            b = Blockly.PHP.provideFunction_("math_standard_deviation",
                ["function " + Blockly.PHP.FUNCTION_NAME_PLACEHOLDER_ + "($numbers) {", "  $n = count($numbers);", "  if (!$n) return null;", "  $mean = array_sum($numbers) / count($numbers);", "  foreach($numbers as $key => $num) $devs[$key] = pow($num - $mean, 2);", "  return sqrt(array_sum($devs) / (count($devs) - 1));", "}"]);
            a = Blockly.PHP.valueToCode(a, "LIST", Blockly.PHP.ORDER_NONE) || "[]";
            a = b + "(" + a + ")";
            break;
        case "RANDOM":
...
Blockly.PHP.procedures_defreturn = function (a) {
    ...
    e = Blockly.PHP.statementToCode(a, "STACK");
    Blockly.PHP.STATEMENT_PREFIX && (f = a.id.replace(/\$/g, "$$$$"), e = Blockly.PHP.prefixLines(Blockly.PHP.STATEMENT_PREFIX.replace(/%1/g, "'" + f + "'"), Blockly.PHP.INDENT) + e);
    Blockly.PHP.INFINITE_LOOP_TRAP && (e = Blockly.PHP.INFINITE_LOOP_TRAP.replace(/%1/g, "'" + a.id + "'") + e);
    (c = Blockly.PHP.valueToCode(a, "RETURN", Blockly.PHP.ORDER_NONE) || "") && (c = Blockly.PHP.INDENT + "return " + c + ";\n");
    var g = [];
    for (f = 0; f < a.arguments_.length; f++) g[f] = Blockly.PHP.variableDB_.getName(a.arguments_[f],
...
Blockly.PHP.text_join = function (a) {
    ...
    for (var c = 0; c < a.itemCount_; c++) b[c] = Blockly.PHP.valueToCode(a, "ADD" +
...
Blockly.PHP.text_indexOf = function (a) {
    ...
    if (a.workspace.options.oneBasedIndex) var e = " 0", f = " + 1"; else e = " -1", f = "";
